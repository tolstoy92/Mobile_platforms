...
if __name__ == "__main__":
    vrep_con = Vrep()
    floorHandle = vrep_con.get_object_handle('floor')
    boundaries = vrep_con.get_boundary_points(floorHandle)
    rows = 13
    cols = 13
    cellsList = vrep_con.create_mesh(rows, cols)
    for mas in cellsList:
        for item in mas:
            print(item.get_xy())
